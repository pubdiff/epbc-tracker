...
let cache: Map<string, ReferralIndexed[]> | null = null;

async function getByJurisdiction(): Promise<Map<string, ReferralIndexed[]>> {
...
export async function generateStaticParams() {
  const map = await getByJurisdiction();
  return [...map.keys()].map((code) => ({ slug: code.toLowerCase() }));
}
...
export async function generateMetadata({ params }: ParamsArg): Promise<Metadata> {
  ...
  return {
    title: `${name} (${code}) referrals`,
    description: `EPBC Act referrals in ${name}.`,
  };
}

export default async function StatePage({ params }: ParamsArg) {
  const { slug } = await params;
  const code = slug.toUpperCase();
  const map = await getByJurisdiction();
  const referrals = map.get(code);
  if (!referrals || referrals.length === 0) notFound();
  ...
  const categoryRows = [...byCategory.entries()].sort((a, b) => b[1] - a[1]).slice(0, 8);
    ...
    <article className="space-y-10">
      <header>
        ...
        <h1 className="text-3xl font-semibold mt-1">
          {name} <span className="text-[var(--color-muted)] font-normal text-2xl">({code})</span>
        </h1>
      ...
      <section>
        <div className="flex items-baseline justify-between mb-3 flex-wrap gap-2">
          ...
          <Link
            href={`/all/?j=${encodeURIComponent(code)}`}
            className="text-sm"
          >
            Browse all {total.toLocaleString()} {code} referrals in filter view →
          </Link>
        </div>
      ...
      <section>
        ...
        <div className="grid sm:grid-cols-2 gap-6 text-sm">
            ...
            <ul className="space-y-1">
              {stageRows.map(([s, n]) => (
                <li key={s} className="flex justify-between gap-3">
                  <Link
                    href={`/all/?j=${encodeURIComponent(code)}&stage=${encodeURIComponent(s)}`}
                    className="truncate"
                  >
                    {s}
                  </Link>
          ...
          <div>
            <h3 className="font-semibold mb-2">By category (top 8)</h3>
            <ul className="space-y-1">
              {categoryRows.map(([c, n]) => (
                <li key={c} className="flex justify-between gap-3">
                  <Link
                    href={`/all/?j=${encodeURIComponent(code)}&c=${encodeURIComponent(c)}`}
                    className="truncate"
                  >
                    {c}
                  </Link>
